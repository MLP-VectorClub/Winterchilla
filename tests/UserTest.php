...
class UserTest extends TestCase {
	function testGetProfileLink(){
		...
		$result = $User->getProfileLink(App\User::LINKFORMAT_FULL);
	...
	function testGetDALink(){
		$User = new App\User([
			'name' => 'TestUser',
			'avatar_url' => '/img/guest.svg',
		]);
		$result = $User->getDALink(App\User::LINKFORMAT_URL);
		static::assertEquals("http://testuser.deviantart.com/", $result, "Testing URL format return value");
		$result = $User->getDALink(App\User::LINKFORMAT_TEXT);
