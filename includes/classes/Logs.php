	...
	public static function formatEntryDetails($MainEntry, $data){
		...
		switch ($reftype){
			...
			case 'cm_modify':
				...
				$olddata = !empty($data['olddata']) ? JSON::encode($data['olddata'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : '';
				$newdata = !empty($data['newdata']) ? JSON::encode($data['newdata'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : '';
				if ($olddata || $newdata){
